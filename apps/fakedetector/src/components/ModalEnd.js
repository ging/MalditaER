...
export default function ModalEnd(props) {
    const localCloseModal = () => {
      if(props.passed){
        console.log("We don´t allow to close the modal. Challenge completed");
      } else {
        props.closeModal();
    ...
    return (
      <Modal  className="modal_content" isOpen={props.showModal} shouldCloseOnOverlayClick={!props.passed} onRequestClose={localCloseModal} style={customStyles} >
        
        <div className="modal_info">
          <h2>{props.I18n.getTrans("i.modalheader2")}</h2>
          <p>{props.passed ? props.I18n.getTrans("i.congrats"):props.I18n.getTrans("i.retry")}</p>
